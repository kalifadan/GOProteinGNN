...
from src.dataset import ProteinGoInputFeatures, GoGoInputFeatures, ProteinSeqInputFeatures, GCNProteinGoInputFeatures
...
def _collate_batch_for_gcn_protein_go(
        examples: List[GCNProteinGoInputFeatures],
        protein_tokenizer: PreTrainedTokenizerBase,
        text_tokenizer: PreTrainedTokenizerBase,
        are_protein_length_same: bool,
        use_pfi: bool
):
    assert isinstance(examples[0], GCNProteinGoInputFeatures), "Only support `GCNProteinGoInputFeatures`"

    use_seq = False
    use_desc = False

    # go_input_ids is a list of list of int (per element each id represents a word in input go sentence)
    if not isinstance(examples[0].go_input_ids[0], int):
        use_desc = True

    # protein_input_ids is list of int each id represent an amino acid in protein seq
    if not isinstance(examples[0].protein_input_ids, int):
        use_seq = True

    # collate positive samples
    # protein
    if use_seq:
        # use sequence
        all_postive_protein_input_ids = [torch.tensor(example.protein_input_ids, dtype=torch.long) for example
                                         in examples]  # list -> tensor
        if are_protein_length_same:
            all_postive_protein_input_ids = torch.stack(all_postive_protein_input_ids, dim=0)
        # protein sequence padding, pad id = 0
        else:
            max_length = max(x.size(0) for x in all_postive_protein_input_ids)
            all_postive_protein_input_ids_ = all_postive_protein_input_ids[0].new_full(
                [len(all_postive_protein_input_ids), max_length], fill_value=protein_tokenizer.pad_token_id)
            for i, postive_protein_input_ids in enumerate(all_postive_protein_input_ids):
                if protein_tokenizer.padding_side == 'right':
                    all_postive_protein_input_ids_[i, :postive_protein_input_ids.size(0)] = postive_protein_input_ids
                else:
        ...
        # not use sequence
        all_postive_protein_input_ids = torch.tensor([example.protein_input_ids for example in examples],
                                                     dtype=torch.long)

    # relation
    relation_ids = [torch.tensor(example.relation_ids, dtype=torch.long) for example in examples]
    padded_relation_ids = relation_ids
    relation_attention_mask = None
    relation_token_type_ids = None

    # For future use - pad number of relations per protein to the maximum number of relations within the batch
    # padded_relation_ids = pad_sequence(relation_ids, batch_first=True, padding_value=0)
    # relation_attention_mask = (padded_relation_ids != text_tokenizer.pad_token_id).long()
    # relation_token_type_ids = torch.zeros_like(padded_relation_ids, dtype=torch.long)

    # go term
    go_input_ids = [torch.tensor(example.go_input_ids, dtype=torch.long) for example in examples]
    padded_go_input_ids = pad_sequence(go_input_ids, batch_first=True, padding_value=0)

    # Create the attention mask and token type IDs
    go_input_attention_mask = (padded_go_input_ids != text_tokenizer.pad_token_id).long()
    go_input_token_type_ids = torch.zeros_like(padded_go_input_ids, dtype=torch.long)

    # if use_desc:
    #     all_postive_go_input_ids = [torch.tensor(example.go_input_ids, dtype=torch.long) for example in examples]
    #     all_postive_go_input_ids = torch.stack(all_postive_go_input_ids, dim=0)

    # else:
    #    all_postive_go_input_ids = torch.tensor([example.go_input_ids for example in examples], dtype=torch.long)
    ...
    return {
        'protein_input_ids': all_postive_protein_input_ids,
        'relation_ids': padded_relation_ids,
        'relation_attention_mask': relation_attention_mask,
        'relation_token_type_ids': relation_token_type_ids,
        'postive': {
            'tail_input_ids': padded_go_input_ids,
            'tail_attention_mask': go_input_attention_mask,
            'tail_token_type_ids': go_input_token_type_ids
        },
    }
    ...
    def __call__(
        ...
        if self.mlm:
            batch['protein_input_ids'], batch['protein_labels'] = self.mask_tokens(
                batch['protein_input_ids'], special_tokens_mask=special_tokens_mask
            )
        else:
        ...
        batch['protein_token_type_ids'] = torch.zeros_like(batch['protein_input_ids'], dtype=torch.long)
...
@dataclass
class DataCollatorForGCNProteinGo:
    """
    Data collator used for KE model which the type of dataset is `ProteinGoDataset`
    ...
    def __call__(
        ...
        batch = _collate_batch_for_gcn_protein_go(examples, self.protein_tokenizer, self.text_tokenizer,
                                                  self.are_protein_length_same, use_pfi=self.use_pfi)
        # special_tokens_mask always None
        special_tokens_mask = batch.pop('special_tokens_mask', None)
        if self.mlm:
            batch['protein_input_ids'], batch['protein_labels'] = self.mask_tokens(
