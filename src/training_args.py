...
class KMAEModelArguments:
    ...
    knowledge_model_file_name: str = field(
        default=None,
        metadata={"help": "The directory of the pretrained KnowledgeBert model"}
    )
    gcn_include_relations: bool = field(
        default=True,
        metadata={"help": "Indicate if to add relations (edge_type) to the GCN"}
    )
    knowledge_layers_step: int = field(
        default=10,
        metadata={"help": "The step (amount of layer) to apply GCN Knowledge during the Encoder forward"}
    )
