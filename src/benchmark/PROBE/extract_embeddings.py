...
class ProteinSeqDataset(Dataset):
    ...
    def __init__(
        ...
        if csv:
            self.protein_seq = pd.read_csv(file_path)
        else:
            df=pd.read_csv(file_path,delimiter="\t",header=None)
            df = df.drop(2,axis=1)
            df = df.rename(columns={0: 'PDB_ID', 1: 'Sequence'})
            self.protein_seq=df
    ...
    def __getitem__(self, index):
        item = self.protein_seq["Sequence"][index]
...
class Seq2Vec(nn.Module):
    ...
    def forward(
        ...
        # Use cls token for model with pfi task
...
def generate_PROBE_embedding(file_path: str,
    tokenizer_path: str,
    pretrained_model_path: str,
    embedding_save_path: str,
    csv: bool=True
    ...
    # list of embedding in the same order as input file
    protein_embeddings = []
    for item in tqdm.tqdm(protein_seq_dataloader):
        _ = item.pop('labels')
        inputs = to_device(item)
    ...
    protein_embeddings = torch.cat(protein_embeddings,dim=0).squeeze().numpy()

    # save protein embeddings
    if csv:
        output_csv = pd.DataFrame(df['Entry'])
    else:
        output_csv = pd.DataFrame(df['PDB_ID'])
    output_csv[[str(i) for i in range(1024)]] = protein_embeddings
    output_csv.to_csv(embedding_save_path, index=False)


if __name__ == '__main__':
    pretrained_model_path = "gcn-output/pretrained/GOProteinGCN/checkpoint-50000/encoder"

    # change the model path to evaluate different models
    generate_PROBE_embedding('datasets/PROBE/protein_sequence_records_df.csv',
                             "Rostlab/prot_bert",
                             pretrained_model_path,
                             'gcn-output/embeddings/GOProteinGCN_human_embedding-full-dataset.csv',
                             True
                             )

    generate_PROBE_embedding('datasets/PROBE/SKEMPI_seq.csv',
                             "Rostlab/prot_bert",
                             pretrained_model_path,
                             'gcn-output/embeddings/GOProteinGCN_SKEMPI_embedding-full-dataset.csv',
                             False
                             )
