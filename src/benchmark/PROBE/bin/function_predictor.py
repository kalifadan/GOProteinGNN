...
def MultiLabelSVC_cross_val_predict(representation_name, dataset, X, y, classifier):
    ...
    if detailed_output:
        with open(r"src/benchmark/PROBE/results/Ontology_based_function_prediction_{1}_{0}_model.pkl".format(representation_name,dataset.split(".")[0]),"wb") as file:
...
def ProtDescModel():   
    ...
    datasets = os.listdir(r"datasets/PROBE/auxilary_input/GO_datasets")
    ...
    for dt in tqdm(filtered_datasets,total=len(filtered_datasets)):
        ...
        dt_file = pd.read_csv(r"datasets/PROBE/auxilary_input/GO_datasets/{0}".format(dt),sep="\t")
        ...
        if detailed_output:
            predictions.to_csv(r"src/benchmark/PROBE/results/Ontology_based_function_prediction_{1}_{0}_predictions.tsv".format(representation_name,dt.split(".")[0]),sep="\t",index=None)
...
def pred_output():
    ...
    if detailed_output:
        df_cv_result.to_csv(r"src/benchmark/PROBE/results/Ontology_based_function_prediction_5cv_{0}.tsv".format(representation_name),sep="\t",index=None)
    ...
    df_cv_mean_result.to_csv(r"src/benchmark/PROBE/results/Ontology_based_function_prediction_5cv_mean_{0}.tsv".format(representation_name),sep="\t",index=None)
...
#save std deviation of scores to file
    ...
    df_cv_std_result.to_csv(r"src/benchmark/PROBE/results/Ontology_based_function_prediction_5cv_std_{0}.tsv".format(representation_name),sep="\t",index=None)
