"""
    Based on https://github.com/lucidrains/ema-pytorch
"""
import copy
import torch
...
class EMA(nn.Module):
    ...
    def update_moving_average(self, ma_model, current_model):
        ...
        for (name, current_params), (_, ma_params) in zip(self.get_params_iter(current_model), self.get_params_iter(ma_model)):
            ...
            current_type = current_params.data.dtype
            current_device = current_params.data.device
            ma_params.data = ma_params.data.to(current_device).to(torch.float32)
            ma_params.data.lerp_(current_params.data.float(), torch.tensor(1. - current_decay).to(current_device))
            ma_params.data = ma_params.data.to(current_type)
        ...
        for (name, current_buffer), (_, ma_buffer) in zip(self.get_buffers_iter(current_model), self.get_buffers_iter(ma_model)):
            ...
            current_type = current_buffer.data.dtype
            current_device = current_buffer.data.device
            ma_buffer.data = ma_buffer.data.to(current_device).to(torch.float32)
            ma_buffer.data.lerp_(current_buffer.data.float(), torch.tensor(1. - current_decay).to(current_device))
            ma_buffer.data = ma_buffer.data.to(current_type)
