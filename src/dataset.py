...
def get_gcn_triplet_data(data_path):
    # input is protein_go_train triplet
    protein_data = collections.OrderedDict()

    for line in open(data_path, 'r'):
        head, relation, tail = [int(protein_id) for protein_id in line.rstrip('\n').split()]

        if head not in protein_data:
            protein_data[head] = {'relations': [], 'tails': []}

        protein_data[head]['relations'].append(relation)
        protein_data[head]['tails'].append(tail)
...
@dataclass
class GCNProteinGoInputFeatures:
    """
    A single set of feature of data for GOProteinGCN pretrain.
    """
    protein_input_ids: List[int]
    relation_ids: List[int]
    go_input_ids: List[List[int]]
...
class ProteinGoDataset(Dataset):
    ...
    def _load_data(self):
        ...
        self.go_types = {idx: line.rstrip('\n') for idx, line in enumerate(open(os.path.join(self.data_dir,
                                                                                             'go_type.txt'), 'r'))}
        self.protein_seq = [line.rstrip('\n') for line in open(os.path.join(self.data_dir,
                                                                            'protein_seq.txt'), 'r')]  # avg protein len ~360
        ...
        # for negative sample. true_tail is a dict, key is (head,relation) value is list of tails.
        # True head: key is (relation,tail)
        self.protein_heads, self.pg_relations, self.go_tails, self.true_tail, self.true_head = get_triplet_data(
            data_path=os.path.join(self.data_dir, 'protein_go_train_triplets_v2.txt')
        )

        assert len(self.protein_heads) == len(self.pg_relations) and len(self.pg_relations) == len(self.go_tails), \
            "error with dataloading, the number of samples of protein,relation,go do not match"
    ...
    def __getitem__(self, index):
        ...
        relation_input_ids = self.text_tokenizer.encode(relation_str, max_length=55, truncation=True,
                                                        padding='max_length')
        ...
        if self.use_desc:
            ...
            go_input_ids = self.text_tokenizer.encode(go_desc, max_length=self.max_text_seq_length, truncation=True,
                                                      padding='max_length')  # max_text_len 128
...
class GCNProteinGoDataset(Dataset):
    """
    Dataset for GCN Protein-GO triplet.
    ...
    def _load_data(self):
        ...
        relation_id = [line.rstrip('\n').split('\t') for line in
                       open(os.path.join(self.data_dir, 'relation2id.txt'), 'r')]
        id2relation_dict = {}
        for i in range(len(relation_id)):
            id2relation_dict[relation_id[i][1]] = relation_id[i][0].replace('_', ' ').replace('|', ' ')
        self.id2relation = id2relation_dict
        ...
        # get all protein heads with their matching relations and tails
        self.protein_heads = get_gcn_triplet_data(
            data_path=os.path.join(self.data_dir, 'protein_go_train_triplets_v2.txt')
        )

    def __getitem__(self, index):
        protein_head_id = list(self.protein_heads.keys())[index]
        relation_ids, go_tail_ids = self.protein_heads[protein_head_id]["relations"], self.protein_heads[protein_head_id]["tails"]

        # Now we use the relations as edge type, but we can take the relation meaning into account as well
        # relation_input_ids = [self.text_tokenizer.encode(self.id2relation[str(relation_id)],
        #                                                  max_length=55, truncation=True, padding='max_length')
        ...
        go_input_ids = go_tail_ids
        if self.use_desc:
            go_descriptions = [self.go_descs[go_tail_id] for go_tail_id in go_input_ids]
            go_input_ids = [self.text_tokenizer.encode(go_desc, max_length=self.max_text_seq_length, truncation=True,
                                                       padding='max_length') for go_desc in go_descriptions]  # max_text_len 128

        return GCNProteinGoInputFeatures(
