...
from src.dataset import GoGoDataset, GCNProteinGoDataset, ProteinSeqDataset
from src.dataloader import DataCollatorForLanguageModeling, DataCollatorForGoGo, DataCollatorForGCNProteinGo
from src.gcn_models import GOProteinGCN, GOProteinGCNLoss
...
class GOProteinGCNTrainer(Trainer):
    """
    GOProteinGCN implement the pretraining of protein language model with knowledge injection, which class
    ...
    def __init__(
        ...
        protein_go_dataset: GCNProteinGoDataset = None,
        go_go_dataset: GoGoDataset = None,
        protein_seq_data_collator: DataCollatorForLanguageModeling = None,
        protein_go_data_collator: DataCollatorForGCNProteinGo = None,
        ...
        self.model_loss = GOProteinGCNLoss(pfi_weight=self.args.pfi_lambda, mlm_lambda=self.args.mlm_lambda,
                                           num_protein_go_neg_sample=self.args.num_protein_go_neg_sample)

        # An option to use EMA model over the GOProteinGNN
        # self.ema_model = EMA(
        #     model,
        #     beta=0.99,              # exponential moving average factor
        #     update_after_step=100,    # only after this number of .update() calls will it start updating
        #     update_every=64,         # how often to actually update, to save on compute
        # )

        if args.local_rank == -1 or args.local_rank == 0:    # only on main process
            wandb.init(
                project="GOProteinGCN",
    ...
    def train(
        ...
        **kwargs,
        ...
        for step in train_iterator:
            ...
            protein_go_inputs = next(protein_go_iter) if protein_go_iter else None
            ...
            if args.local_rank == -1 or args.local_rank == 0:
                ...
                # log to wandb the current loss
                metrics_to_log = all_loss.copy()
                if isinstance(metrics_to_log['learning_rate'], list):
                    metrics_to_log['learning_rate'] = float(metrics_to_log['learning_rate'][0])
                wandb.log(metrics_to_log)

            # Optimizer step for deepspeed must be called on every step regardless to the gradient_accumulation_steps
            if self.deepspeed:
    ...
    def _save_checkpoint(self):
        ...
        if self.deepspeed:
            logger.info(f"Saving deepspeed checkpoint to {output_dir}")
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        # Save EMA model
        # self.ema_model.ema_model.save_pretrained(f"{output_dir}-EMA")
    ...
    def training_step(
        ...
        self.use_amp = False    # AMP is not supported in these torch's versions
    ...
    def compute_loss(
        ...
        if protein_go_inputs:
            ...
            mlm_loss, positive_loss, negative_loss = self.model_loss(model=model,
                                                                     use_desc=self.args.use_desc,
                                                                     global_step=self.state.global_step,
                                                                     use_pfi=self.args.use_pfi,
                                                                     protein_go_inputs=protein_go_inputs)
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            encoder_parameters = get_parameter_names(self.model, [GOProteinGCN])
            ...
            print(f"Chosen lr: {self.args.lm_learning_rate}, Chosen weight decay: {self.args.weight_decay}")
    ...
    def create_scheduler(self, num_training_steps: int, optimizer: torch.optim.Optimizer = None):
        ...
        Note: It is override from `transformer.Trainer.create_scheduler`.

        Args:
            num_training_steps (int): The number of training steps to do.
            optimizer (Optimizer): The optimizer of the trainer must have been set up before this method is called.
                                    The parameter is not used and it's for compatibility with the Transformers library.
        """
