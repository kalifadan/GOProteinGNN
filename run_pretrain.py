...
from src.gcn_models import GOProteinGCN
from src.gcn_trainer import GOProteinGCNTrainer

from src.sampling import negative_sampling_strategy
from src.dataset import GCNProteinGoDataset
from src.dataloader import DataCollatorForGoGo, DataCollatorForLanguageModeling, DataCollatorForGCNProteinGo
...
def main():
    ...
    if data_args.model_protein_go_data:
        protein_go_dataset = GCNProteinGoDataset(
    ...
    protein_seq_data_collator = DataCollatorForLanguageModeling(tokenizer=protein_tokenizer,
                                                                are_protein_length_same=are_protein_length_same)
    go_go_data_collator = DataCollatorForGoGo(tokenizer=text_tokenizer)

    protein_go_data_collator = DataCollatorForGCNProteinGo(protein_tokenizer=protein_tokenizer,
                                                           text_tokenizer=text_tokenizer,
                                                           are_protein_length_same=are_protein_length_same,
                                                           use_pfi=use_pfi)

    model = GOProteinGCN.from_pretrained(
        protein_model_path=model_args.encoder_model_file_name,
        text_model_path=model_args.text_model_file_name,
        knowledge_model_file_name=model_args.knowledge_model_file_name,
    ...
    if training_args.do_train:
        ...
        # trainer.train(resume_from_checkpoint="new7-gcn-output/pretrained/GOProteinGCN/10k")
        trainer.train()
